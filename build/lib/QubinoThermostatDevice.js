...
class QubinoThermostatDevice extends QubinoDevice {
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    // If enabled/disabled
    if (changedKeys.includes(SETTINGS.ANTIFREEZE_ENABLED)) {
      ...
      if (!(SETTINGS.ANTIFREEZE in changedKeys)) {
        changedKeys.push(SETTINGS.ANTIFREEZE);
      }
      newSettings[SETTINGS.ANTIFREEZE] = antifreezeValue;
    }

    // If enabled/disabled
    if (changedKeys.includes(SETTINGS.ANTIFREEZE)) {
      const antifreezeEnabledValue = Object.prototype.hasOwnProperty.call(newSettings, SETTINGS.ANTIFREEZE_ENABLED) ? newSettings[SETTINGS.ANTIFREEZE_ENABLED] : oldSettings[SETTINGS.ANTIFREEZE_ENABLED];
