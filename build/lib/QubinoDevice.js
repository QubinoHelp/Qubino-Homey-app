...
class QubinoDevice extends ZwaveDevice {
   ...
   */
  customSaveMessage(oldSettings, newSettings, changedKeys = []) {
    if (changedKeys.includes(SETTINGS.ENABLE_INPUT_1)
      || changedKeys.includes(SETTINGS.ENABLE_INPUT_2)
      || changedKeys.includes(SETTINGS.ENABLE_INPUT_3)
      || changedKeys.includes(SETTINGS.FUNCTIONALITY_INPUT_3)
      || changedKeys.includes(SETTINGS.THERMOSTAT_MODE)
      || changedKeys.includes(SETTINGS.WORKING_MODE)) {
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes(SETTINGS.ALL_ON) || changedKeys.includes(SETTINGS.ALL_OFF)) {
      ...
      changedKeys = [...changedKeys.filter(changedKey => changedKey !== SETTINGS.ALL_ON && changedKey !== SETTINGS.ALL_OFF)];
    }
  ...
  async _configureReporting() {
    if (this.numberOfMultiChannelNodes > 0 && !this.getSetting(SETTINGS.MULTI_CHANNEL_REPORTING_CONFIGURED)) {
      try {
        ...
        this.setUnavailable(this.homey.__('error.missing_multi_channel_command_class'));
      }
