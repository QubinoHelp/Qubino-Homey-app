...
class ZMNHWD extends QubinoDimDevice {
  ...
  async _sendColors({
    ...
    if (SwitchColorVersion === 2) {
      ...
      setCommand = Buffer.from([setCommand.Properties1['Color Component Count'], 0, setCommand.vg1[0].Value, 2, setCommand.vg1[1].Value, 3, setCommand.vg1[2].Value, 4, setCommand.vg1[3].Value]);
    }
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes(SETTINGS.AUTO_SCENE_MODE_TRANSITION_DURATION_UNIT)) {
      ...
      if (!changedKeys.includes(SETTINGS.AUTO_SCENE_MODE_TRANSITION_DURATION)) {
        changedKeys.push(SETTINGS.AUTO_SCENE_MODE_TRANSITION_DURATION);
      }
    }
