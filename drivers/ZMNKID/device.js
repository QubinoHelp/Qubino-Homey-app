...
class ZMNKID extends QubinoThermostatDevice {
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    if (changedKeys.includes(SETTINGS.POWER_REPORTING_INTERVAL)) {
      ...
      if (newSettings[SETTINGS.POWER_REPORTING_INTERVAL] !== 0 && newSettings[SETTINGS.POWER_REPORTING_INTERVAL] < 30) {
        throw new Error(this.homey.__('settings.power_reporting_value_too_low'));
      }
    }
  ...
  async registerCapabilities() {
    ...
    this.registerCapability(CAPABILITIES.OFF_AUTO_THERMOSTAT_MODE, COMMAND_CLASSES.THERMOSTAT_MODE, {
      ...
      setParser: mode => {
        this.driver.triggerFlow(FLOWS.OFF_AUTO_THERMOSTAT_MODE_CHANGED, this, {}, { mode }).catch(err => this.error('failed to trigger flow', FLOWS.OFF_AUTO_THERMOSTAT_MODE_CHANGED, err));
        return {
          Level: {
