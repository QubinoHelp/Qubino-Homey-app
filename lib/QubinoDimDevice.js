...
class QubinoDimDevice extends QubinoDevice {
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes(SETTINGS.MAXIMUM_DIM_VALUE) || changedKeys.includes(SETTINGS.MINIMUM_DIM_VALUE)) {
      ...
      if (maxDimValue < minDimValue) {
        return Promise.reject(new Error(this.homey.__('error.max_dim_level_cannot_be_lower_than_min_dim_level')));
      }
    }
